...
var template = '<div class="freewriting-toolbar" id ="toolbar">' + '<span id="toolbarWrapper">Tid: <span id="timerSettings"><span id="timer"><span id="minutes">10</span>:00 </span><span id="plusFive" class="button"><span class="dashicons dashicons-admin-post"></span></span><span id="minusFive" class="button">-</span></span><span id="wordCount" style="float: right;"></span></div>' + '</div>' + '<div class="freewriting-text-box">' + '<textarea id="textInput" rows="14" placeholder="Du starter programmet, når du skriver her.."></textarea>' + '<div id="placeholder"></div>' + '</div>';
render(template, '#app');

/*
A function to set the timer before launch
*/
var setTimer = function(e) {
  // If click is actually on the setTimer
  var minutes = document.getElementById('minutes');
  var currentTimer = parseInt(minutes.innerHTML);
  if (e.target.id === 'plusFive') {
    minutes.innerHTML = currentTimer + 5;
  } else if (e.target.id === 'minusFive') {
    currentTimer = parseInt(minutes.innerHTML);
    if (currentTimer > 5) {
      minutes.innerHTML = currentTimer - 5;
    }
  }
...
var initiator = function(e) {
  ...
  if (e.target.id === 'textInput') {
    ...
    var workPeriodInSeconds = 60 * currentTimer;
...
document.addEventListener('click', setTimer, false);
